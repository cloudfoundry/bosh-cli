	...
	Context("when a CPI release exists", func() {
		...
		Context("when a manifest exists", func() {
			BeforeEach(func() {
				...
				err = bitestutils.GenerateDeploymentManifest(deploymentManifestFilePath, fileSystem, "---")
				Expect(err).NotTo(HaveOccurred())
			})
			...
			It("does not set up the workspace if there has not been a deployment", func() {
				session, err := bitestutils.RunBoshMicro("deploy")
			...
			It("says the current deployment is set", func() {
				session, err := bitestutils.RunBoshMicro("deployment", deploymentManifestFilePath)
				Expect(err).NotTo(HaveOccurred())
				Expect(session.ExitCode()).To(Equal(0))

				session, err = bitestutils.RunBoshMicro("deployment")
				Expect(err).NotTo(HaveOccurred())
				Expect(session.ExitCode()).To(Equal(0))
		...
		Context("when no manifest has been set", func() {
			It("refuses to deploy", func() {
				...
				session, err := bitestutils.RunBoshMicro("deploy", tmpFile.Name(), tmpFile.Name())
				Expect(err).NotTo(HaveOccurred())
