...
func toDependencies(relationships []artifact.Relationship) []cyclonedx.Dependency {
	...
	groupedRelationships := make(map[string][]string)
	for _, r := range relationships {
		...
		groupedRelationships[deriveBomRef(*toPkg)] = append(groupedRelationships[deriveBomRef(*toPkg)], deriveBomRef(*fromPkg))

	}
	for to, from := range groupedRelationships {
		fromCopy := from
		result = append(result, cyclonedx.Dependency{
			Ref:          to,
			Dependencies: &fromCopy,
		})
	}
	return result
}
